...
if __name__ == '__main__':
    # 你的train目录
    train_dir = r'C:\baidunetdiskdownload\train'
    ...
    for seq_name in os.listdir(train_dir):
        ...
        output_video = os.path.join(seq_path, 'output_bbox.mp4')
        save_frames = os.path.join(seq_path, 'img_bbox')
        # 检查img和mask文件夹是否存在
        if not (os.path.isdir(img_dir) and os.path.isdir(mask_dir)):
            continue
        # 调用你的脚本
        cmd = [
            'python', 'draw_bbox_video.py',
            '--images', img_dir,
            '--masks', mask_dir,
            '--output', output_video,
